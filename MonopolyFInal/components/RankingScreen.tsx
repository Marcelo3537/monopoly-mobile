          ...
          <Card
            ...
            className={`border-amber-500/20 backdrop-blur-sm transition-all duration-200 ${
              user.id === currentUser.id 
                ? 'bg-amber-500/20 border-amber-400/50 scale-105' 
          ...
          >
            <CardContent className="p-4">
              <div className="flex items-center space-x-4">
                {/* Rank */}
                <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${getRankColor(user.rank)} flex items-center justify-center shadow-lg`}>
                  {user.rank <= 3 ? (
                ...
                <div className="flex-1">
                  <div className="flex items-center space-x-2">
                    ...
                    {user.id === currentUser.id && (
                      <Badge className="bg-amber-500 text-black text-xs">TÚ</Badge>
                    )}
                  </div>
                  <p className="text-amber-300 text-sm">{user.level}</p>
                ...
                <div className="text-right">
                  <p className="text-purple-300 text-xl">{user.elo}</p>
        ...
        <Card className="bg-black/40 border-amber-500/20">
          <CardContent className="p-3">
            <div className="grid grid-cols-2 gap-4 text-center">
              <div>
                <p className="text-amber-300 text-xs">Tu posición</p>
                <p className="text-white text-lg">#{currentUser.id === '4' ? '4' : 'N/A'}</p>
              </div>
              <div>
                <p className="text-green-300 text-xs">Total jugadores</p>
                <p className="text-white text-lg">{rankings.length * 100}+</p>
              </div>
            </div>
          </CardContent>
        </Card>
