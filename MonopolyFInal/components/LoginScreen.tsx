...
export function LoginScreen({ onLogin }: LoginScreenProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (username.trim()) {
      try {
        ...
        alert('❌ No se pudo conectar con el servidor. Asegúrate de que el backend está ejecutándose en el puerto 3001.');
      }
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-red-900 via-red-800 to-amber-900 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      {/* Tarjeta principal */}
      <Card className="w-full max-w-sm bg-black/80 border-amber-500/30 shadow-2xl backdrop-blur-sm relative z-10">
        <CardHeader className="text-center pb-2">
          <div className="flex items-center justify-center mb-4">
            <div className="w-16 h-16 bg-gradient-to-br from-amber-400 to-red-600 rounded-full flex items-center justify-center">
              <Crown className="w-8 h-8 text-white" />
            </div>
          </div>
          <CardTitle className="text-3xl bg-gradient-to-r from-amber-400 to-red-500 bg-clip-text text-transparent">
            Casino y Tapas
          </CardTitle>
